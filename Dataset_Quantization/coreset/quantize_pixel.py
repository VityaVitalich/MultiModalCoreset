...
def get_args_parser():
    ...
    parser.add_argument(
        "--data_path", default="../data_cifar", type=str, help="dataset path"
    )
    parser.add_argument(
        "--output_dir",
        default="../output/recons_cifar10_base",
    ...
    parser.add_argument(
        "--resume",
        default="./mae_visualize_vit_large_ganloss.pth",
...
def prepare_model(chkpt_dir, args):
    ...
    model = getattr(mae_models, args.model)(
        cam_mask=args.cam_mask, img_size=args.input_size
    )
    # load model
...
if __name__ == "__main__":
    ...
    if args.data == "CIFAR10":
        ...
        dataset_train = ImageFolderWithPaths(
            root=args.data_path, transform=transform_test
        )
    ...
    for idx, data in pbar:
        ...
        for j in range(recovery_img.shape[0]):
            ...
            if args.data == "CIFAR10":
                recovery_img_j = torchvision.transforms.functional.resize(
                    recovery_img_j, [32, 32]
                )
