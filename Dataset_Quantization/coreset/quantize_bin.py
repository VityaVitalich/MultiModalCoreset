...
def main():
    ...
    parser.add_argument(
        "--uncertainty", default="Entropy", help="specifiy uncertanty score to use"
    )
    parser.add_argument(
        "--replace",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="path to previous selection results",
    )
    parser.add_argument(
        "--save_path",
        "-sp",
    ...
    select_indices_list = [
        np.load(os.path.join(args.selection_path, fp)) for fp in select_indices_files
    ]
    ...
    for exp, select_indices in enumerate(select_indices_list):
        ...
        print(
            "Exp: {}, Dst Size: {}, Fraction: {}".format(
                exp, len(sub_dst_train), args.fraction
            )
        )
        method = methods.__dict__[args.selection](
            sub_dst_train, args, args.fraction, args.seed, balance=args.balance
        )
        subset = method.select()
        mapped_indices = select_indices[subset["indices"]]
        print(
            "Exp: {}, Available indices: {}, Select Subset: {}".format(
                exp, len(select_indices), len(mapped_indices)
            )
        )
