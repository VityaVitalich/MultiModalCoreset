...
def main():
    ...
    # Basic arguments
    parser.add_argument("--dataset", type=str, default="CIFAR10", help="dataset path")
    parser.add_argument("--model", type=str, default="ResNet18", help="multimae checkpoint")
    parser.add_argument(
        "--selection", type=str, default="Submodular", help="selection method"
    )
    parser.add_argument(
        "--num_exp", type=int, default=5, help="the number of experiments"
    )
    parser.add_argument(
        "--batch", type=int, default=128, help="the number of batch size for selection"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--print_freq", "-p", default=50, type=int, help="print frequency (default: 20)"
    )
    parser.add_argument(
        "--fraction",
        default=0.1,
    ...
    parser.add_argument(
        ...
        type=float,
        metavar="W",
        help="weight decay whiling performing selection (default: 5e-4)",
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="the number of training epochs to be preformed between two test epochs during selection (default: 1)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether the samples can be selected repeatedly",
    )
    parser.add_argument(
        "--pretrained",
        action="store_true",
        default=False,
        help="whether the select model is pretrained",
    )

    # Algorithm
    parser.add_argument(
        "--submodular", default="GraphCut", help="specifiy submodular function to use"
    )
    parser.add_argument(
        "--submodular_greedy",
        default="NaiveGreedy",
    ...
    for exp in range(args.num_exp):
        ...
        print(
            "Exp: {}, Dst Size: {}, Fraction: {}".format(exp, len(dst_train), fraction)
        )

        # selection fraction samples from the remaining indices
        method = methods.__dict__[args.selection](
            dst_train, args, fraction, args.seed, **selection_args
        )
        subset = method.select()
        mapped_indices = avail_indices[subset["indices"]]
        print(
            "Exp: {}, Available indices: {}, Select Subset: {}".format(
                exp, len(avail_indices), len(mapped_indices)
            )
        )
        ...
        if (
            not args.replace
            and exp == args.num_exp - 2
            and abs(args.fraction * args.num_exp - 1) < 1e-5
        ):
            select_save_path = os.path.join(
                args.save_path,
                "select_indices_{}_exp{}.npy".format(args.dataset, exp + 1),
            )
