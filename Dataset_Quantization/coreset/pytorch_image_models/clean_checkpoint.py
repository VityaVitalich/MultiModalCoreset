...
parser.add_argument(
    "--output", default="", type=str, metavar="PATH", help="output path"
)
parser.add_argument(
    "--no-use-ema",
    dest="no_use_ema",
...
def main():
    ...
    clean_checkpoint(
        args.checkpoint, args.output, not args.no_use_ema, args.clean_aux_bn
    )
...
def clean_checkpoint(checkpoint, output="", use_ema=True, clean_aux_bn=False):
    ...
    if checkpoint and os.path.isfile(checkpoint):
        ...
        print(
            "=> Saved state_dict to '{}, SHA256: {}'".format(final_filename, sha_hash)
        )
