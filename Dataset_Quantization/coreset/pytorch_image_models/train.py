...
parser.add_argument("train_data_dir", metavar="DIR", help="train dataset root")
parser.add_argument("eval_data_dir", metavar="DIR", help="eval dataset root")
group.add_argument(
    "--dataset",
    "-d",
    metavar="NAME",
...
def main():
    ...
    if len(args.select_indices) > 0:
        select_indices = np.array([]).astype(np.int)
        for indices in args.select_indices:
            select_indices = np.append(select_indices, np.load(indices))
        dataset_train = torch.utils.data.Subset(dataset_train, select_indices)
    ...
    if args.rank == 0:
        if args.experiment:
            ...
            if len(args.select_indices) > 0:
                index_names = "-".join(
                    [index.split("/")[-1][:-4] for index in args.select_indices]
                )
                exp_name = "-".join(
                    [
