...
def _test_basic_cases(constructor, scheduler_constructors=None):
    ...
    _test_state_dict(torch.randn(10, 5), torch.randn(10), torch.randn(5), constructor)
...
def _test_model(optimizer, params, device=torch.device("cpu")):
    ...
    weight2 = torch.tensor(
        [[-0.0508, -0.3941, -0.2843]], device=device, requires_grad=True
    )
...
def test_sgd(optimizer):
    ...
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict(weight, bias, lr=1e-2), optimizer, lr=1e-3
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict_single(weight, bias, lr=1e-2), optimizer, lr=1e-3
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
    ...
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            [weight, bias], optimizer, lr=3e-3, momentum=1
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            [weight, bias], optimizer, lr=3e-3, momentum=1, weight_decay=0.1
        )
    )
    _test_rosenbrock(lambda params: create_optimizer_v2(params, optimizer, lr=1e-3))
...
def test_adam(optimizer):
    ...
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict(weight, bias, lr=3e-3), optimizer, lr=1e-3
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict_single(weight, bias, lr=3e-3), optimizer, lr=1e-3
        )
    )
    _test_rosenbrock(lambda params: create_optimizer_v2(params, optimizer, lr=5e-2))
...
def test_adabelief(optimizer):
    ...
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            [weight, bias], optimizer, lr=1e-3, weight_decay=1
        )
    )
    _test_rosenbrock(lambda params: create_optimizer_v2(params, optimizer, lr=5e-2))
    _test_model(optimizer, dict(lr=5e-2))
...
def test_adaother(optimizer):
    ...
    _test_rosenbrock(lambda params: create_optimizer_v2(params, optimizer, lr=1e-1))
    _test_model(optimizer, dict(lr=5e-2))
...
def test_adafactor(optimizer):
    ...
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict_single(weight, bias), optimizer
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
...
def test_lamb(optimizer):
    ...
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict(weight, bias, lr=1e-3), optimizer, lr=1e-3
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
            _build_params_dict_single(weight, bias, lr=1e-3), optimizer, lr=1e-3
        )
    )
    _test_basic_cases(
        lambda weight, bias: create_optimizer_v2(
...
def test_madgrad(optimizer):
    ...
    _test_rosenbrock(lambda params: create_optimizer_v2(params, optimizer, lr=1e-2))
...
def test_lookahead_radam(optimizer):
    ...
    _test_rosenbrock(lambda params: create_optimizer_v2(params, optimizer, lr=1e-4))
