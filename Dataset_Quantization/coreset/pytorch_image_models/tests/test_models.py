...
@pytest.mark.parametrize(
    "model_name", list_models(exclude_filters=EXCLUDE_FILTERS, name_matches_cfg=True)
)
...
def test_model_default_cfgs(model_name, batch_size):
    ...
    if all([x <= MAX_FWD_OUT_SIZE for x in input_size]) and not any(
        ...
        assert (
            outputs.shape[-1] == pool_size[-1] and outputs.shape[-2] == pool_size[-2]
        ), "unpooled feature shape != config"
        ...
        model.reset_classifier(
            0, ""
        )  # reset classifier and set global pooling to pass-through
        outputs = model.forward(input_tensor)
        assert len(outputs.shape) == 4
        if not isinstance(
            model, (timm.models.MobileNetV3, timm.models.GhostNet, timm.models.VGG)
        ):
            # mobilenetv3/ghostnet/vgg forward_features vs removed pooling differ due to location or lack of GAP
            assert (
                outputs.shape[-1] == pool_size[-1]
                and outputs.shape[-2] == pool_size[-2]
            )
...
@pytest.mark.parametrize(
    "model_name",
    list_models(filter=NON_STD_FILTERS, exclude_filters=NON_STD_EXCLUDE_FILTERS),
)
@pytest.mark.parametrize("batch_size", [1])
def test_model_default_cfgs_non_std(model_name, batch_size):
    ...
    if feat_dim is None:
        feat_dim = -1 if outputs.ndim == 3 else 1
    assert (
        outputs.shape[feat_dim] == model.num_features
    ), "pooled num_features != config"
...
if "GITHUB_ACTIONS" not in os.environ:
    ...
    def test_model_load_pretrained(model_name, batch_size):
        ...
        in_chans = (
            3 if "pruned" in model_name else 1
        )  # pruning not currently supported with in_chans change
    ...
    @pytest.mark.timeout(120)
    @pytest.mark.parametrize(
        "model_name", list_models(pretrained=True, exclude_filters=NON_STD_FILTERS)
    )
    @pytest.mark.parametrize("batch_size", [1])
...
EXCLUDE_JIT_FILTERS = [
    ...
    "dla*",
    "hrnet*",
    "ghostnet*",  # hopefully fix at some point
    "vit_large_*",
    "vit_huge_*",
    "vit_gi*",
]
...
@pytest.mark.parametrize(
    "model_name",
    list_models(
        exclude_filters=EXCLUDE_FILTERS + EXCLUDE_JIT_FILTERS, name_matches_cfg=True
    ),
)
@pytest.mark.parametrize("batch_size", [1])
...
@pytest.mark.timeout(120)
@pytest.mark.parametrize(
    "model_name", list_models(exclude_filters=EXCLUDE_FILTERS + EXCLUDE_FEAT_FILTERS)
)
@pytest.mark.parametrize("batch_size", [1])
...
def _create_fx_model(model, train=False):
    ...
    if train:
        ...
        output_node_indices = [
            -graph_node_names.index(node_name) for node_name in output_node_names
        ]
...
@pytest.mark.timeout(120)
@pytest.mark.parametrize(
    "model_name", list_models(exclude_filters=EXCLUDE_FILTERS + EXCLUDE_FX_FILTERS)
)
@pytest.mark.parametrize("batch_size", [1])
