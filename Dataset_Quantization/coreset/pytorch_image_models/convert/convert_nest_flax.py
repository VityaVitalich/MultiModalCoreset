...
def convert_nest(checkpoint_path, arch):
    ...
    for level in range(len(depths)):
        for layer in range(depths[level]):
            ...
            for i in range(2):
                state_dict[
                    f"levels.{level}.transformer_encoder.{layer}.norm{i+1}.weight"
                ] = torch.tensor(
                    flax_dict[f"EncoderNDBlock_{global_layer_ix}"][f"LayerNorm_{i}"][
                        "scale"
                    ]
                )
                state_dict[
                    f"levels.{level}.transformer_encoder.{layer}.norm{i+1}.bias"
                ] = torch.tensor(
                    flax_dict[f"EncoderNDBlock_{global_layer_ix}"][f"LayerNorm_{i}"][
                        "bias"
                    ]
            ...
            state_dict[
                f"levels.{level}.transformer_encoder.{layer}.attn.qkv.bias"
            ] = torch.tensor(b_qkv).reshape(-1)
            # Attention proj
            ...
            state_dict[
                f"levels.{level}.transformer_encoder.{layer}.attn.proj.weight"
            ] = w_proj
            state_dict[
                f"levels.{level}.transformer_encoder.{layer}.attn.proj.bias"
            ] = torch.tensor(
                flax_dict[f"EncoderNDBlock_{global_layer_ix}"]["MultiHeadAttention_0"][
                    "bias"
                ]
            )
            # MLP
            for i in range(2):
                state_dict[
                    f"levels.{level}.transformer_encoder.{layer}.mlp.fc{i+1}.weight"
                ] = torch.tensor(
                    flax_dict[f"EncoderNDBlock_{global_layer_ix}"]["MlpBlock_0"][
                        f"Dense_{i}"
                    ]["kernel"]
                ).permute(
                    1, 0
                ...
                state_dict[
                    f"levels.{level}.transformer_encoder.{layer}.mlp.fc{i+1}.bias"
                ] = torch.tensor(
                    flax_dict[f"EncoderNDBlock_{global_layer_ix}"]["MlpBlock_0"][
                        f"Dense_{i}"
                    ]["bias"]
    ...
    for level in range(1, len(depths)):
        ...
        state_dict[f"levels.{level}.pool.conv.weight"] = torch.tensor(
            flax_dict[f"ConvPool_{level-1}"]["Conv_0"]["kernel"]
        ).permute(3, 2, 0, 1)
    ...
    # Classifier
    state_dict["head.weight"] = torch.tensor(flax_dict["Dense_0"]["kernel"]).permute(
        1, 0
    )
