...
class BnActConv2d(nn.Module):
    def __init__(
        self, in_chs, out_chs, kernel_size, stride, groups=1, norm_layer=BatchNormAct2d
    ):
...
class DPN(nn.Module):
    def __init__(
        ...
        fc_norm_layer = partial(
            BatchNormAct2d, eps=0.001, act_layer=fc_act_layer, inplace=False
        )
        ...
        self.feature_info = [
            dict(num_chs=num_init_features, reduction=2, module="features.conv1_1")
        ]
        ...
        for i in range(2, k_sec[0] + 1):
            ...
            in_chs += inc
        self.feature_info += [
            dict(num_chs=in_chs, reduction=4, module=f"features.conv2_{k_sec[0]}")
        ]
        ...
        for i in range(2, k_sec[1] + 1):
            ...
            in_chs += inc
        self.feature_info += [
            dict(num_chs=in_chs, reduction=8, module=f"features.conv3_{k_sec[1]}")
        ]
        ...
        for i in range(2, k_sec[2] + 1):
            ...
            in_chs += inc
        self.feature_info += [
            dict(num_chs=in_chs, reduction=16, module=f"features.conv4_{k_sec[2]}")
        ]
        ...
        for i in range(2, k_sec[3] + 1):
            ...
            in_chs += inc
        self.feature_info += [
            dict(num_chs=in_chs, reduction=32, module=f"features.conv5_{k_sec[3]}")
        ]
