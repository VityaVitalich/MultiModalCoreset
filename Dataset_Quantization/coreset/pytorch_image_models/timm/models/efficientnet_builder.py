...
def round_channels(
    ...
    return make_divisible(
        channels * multiplier, divisor, channel_min, round_limit=round_limit
    )
...
def decode_arch_def(
    ...
    for stack_idx, (block_strings, multiplier) in enumerate(
        ...
        if fix_first_last and (stack_idx == 0 or stack_idx == len(arch_def) - 1):
            ...
            arch_args.append(
                _scale_stage_depth(stack_args, repeats, multiplier, depth_trunc)
            )
    return arch_args
        ...
        for stack_idx, stack_args in enumerate(model_block_args):
            ...
            for block_idx, block_args in enumerate(stack_args):
                ...
                if extract_features:
                    ...
                    feature_info["module"] = (
                        ".".join([module_name, leaf_name]) if leaf_name else module_name
                    )
                    self.features.append(feature_info)
    ...
    * https://github.com/tensorflow/tpu/blob/master/models/official/efficientnet/efficientnet_model.py
    ...
    if isinstance(m, CondConv2d):
        ...
        init_weight_fn = get_condconv_initializer(
            lambda w: nn.init.normal_(w, 0, math.sqrt(2.0 / fan_out)),
            m.num_experts,
            m.weight_shape,
        )
