...
def scale_image(
    x, ss: Tuple[int, int], crop_scale: bool = False
):  # annotations for torchscript
    """
    ...
    if H != ss[0] or W != ss[1]:
        if crop_scale and ss[0] <= H and ss[1] <= W:
            ...
            x = torch.nn.functional.interpolate(
                x, size=ss, mode="bicubic", align_corners=False
            )
    return x
...
class CrossViT(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.num_branches):
            ...
            setattr(
                self, f"cls_token_{i}", nn.Parameter(torch.zeros(1, 1, embed_dim[i]))
            )
    ...
    def reset_classifier(self, num_classes, global_pool=None):
        ...
        self.head = nn.ModuleList(
            [
                nn.Linear(self.embed_dim[i], num_classes)
                if num_classes > 0
                else nn.Identity()
                for i in range(self.num_branches)
            ]
    ...
    def forward_features(self, x) -> List[torch.Tensor]:
        ...
        for i, patch_embed in enumerate(self.patch_embed):
            ...
            x_ = patch_embed(x_)
            cls_tokens = (
                self.cls_token_0 if i == 0 else self.cls_token_1
            )  # hard-coded for torch jit script
            cls_tokens = cls_tokens.expand(B, -1, -1)
            x_ = torch.cat((cls_tokens, x_), dim=1)
            pos_embed = (
                self.pos_embed_0 if i == 0 else self.pos_embed_1
            )  # hard-coded for torch jit script
            x_ = x_ + pos_embed
            x_ = self.pos_drop(x_)
            xs.append(x_)
    ...
    def forward_head(
        self, xs: List[torch.Tensor], pre_logits: bool = False
    ) -> torch.Tensor:
        xs = (
            [x[:, 1:].mean(dim=1) for x in xs]
            if self.global_pool == "avg"
            else [x[:, 0] for x in xs]
        )
        if pre_logits or isinstance(self.head[0], nn.Identity):
            return torch.cat([x for x in xs], dim=1)
        return torch.mean(
            torch.stack([head(xs[i]) for i, head in enumerate(self.head)], dim=0), dim=0
        )
