    ...
    def __init__(
        ...
        if outplanes != inplanes or stride != 1:
            ...
            self.skip.add_module(
                "conv1", nn.Conv2d(inplanes, outplanes, 1, stride=stride, bias=False)
            ),
...
class Xception65(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            in_chans, 32, kernel_size=3, stride=2, padding=1, bias=False
        )
        ...
        self.block1 = Block(
            64, 128, stride=2, start_with_relu=False, norm_layer=norm_layer
        )
        self.block1_act = nn.ReLU(inplace=True)
        self.block2 = Block(
            128, 256, stride=2, start_with_relu=False, norm_layer=norm_layer
        )
        ...
        self.block20 = Block(
            ...
            dilation=exit_dilation[0],
            norm_layer=norm_layer,
        )
        self.block20_act = nn.ReLU(inplace=True)

        self.conv3 = SeparableConv2d(
            1024, 1536, 3, stride=1, dilation=exit_dilation[1], norm_layer=norm_layer
        )
        self.bn3 = norm_layer(num_features=1536)
        self.act3 = nn.ReLU(inplace=True)

        self.conv4 = SeparableConv2d(
            1536, 1536, 3, stride=1, dilation=exit_dilation[1], norm_layer=norm_layer
        )
...
def _create_gluon_xception(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        Xception65, variant, pretrained, feature_cfg=dict(feature_cls="hook"), **kwargs
    )
