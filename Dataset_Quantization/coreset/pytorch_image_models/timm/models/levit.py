...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = ln_layer(
            dim, self.val_attn_dim + self.key_attn_dim * 2, resolution=resolution
        )
        ...
        pos = torch.stack(
            torch.meshgrid(torch.arange(resolution), torch.arange(resolution))
        ).flatten(1)
    ...
    def forward(self, x):  # x (B,C,H,W)
        if self.use_conv:
            ...
            q, k, v = (
                ...
                .split([self.key_dim, self.key_dim, self.val_dim], dim=2)
            )

            attn = (q.transpose(-2, -1) @ k) * self.scale + self.get_attention_biases(
                x.device
            )
            ...
            q, k, v = (
                ...
                .split([self.key_dim, self.key_dim, self.val_dim], dim=3)
            )
            q = q.permute(0, 2, 1, 3)
            k = k.permute(0, 2, 3, 1)
            v = v.permute(0, 2, 1, 3)
...
class AttentionSubsample(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Sequential(
            act_layer(), ln_layer(self.val_attn_dim, out_dim, resolution=resolution_out)
        )

        self.attention_biases = nn.Parameter(
            torch.zeros(num_heads, self.resolution**2)
        )
        k_pos = torch.stack(
            torch.meshgrid(torch.arange(resolution), torch.arange(resolution))
        ).flatten(1)
    ...
    def forward(self, x):
        if self.use_conv:
            ...
            k, v = (
                self.kv(x)
                .view(B, self.num_heads, -1, H * W)
                .split([self.key_dim, self.val_dim], dim=2)
            )
            q = self.q(x).view(
                B, self.num_heads, self.key_dim, self.resolution_out_area
            )
            ...
            x = (v @ attn.transpose(-2, -1)).reshape(
                B, -1, self.resolution, self.resolution
            )
        else:
            B, N, C = x.shape
            k, v = (
                self.kv(x)
                .view(B, N, self.num_heads, -1)
                .split([self.key_dim, self.val_dim], dim=3)
            )
            k = k.permute(0, 2, 3, 1)  # BHCN
            v = v.permute(0, 2, 1, 3)  # BHNC
            q = (
                self.q(x)
                .view(B, self.resolution_out_area, self.num_heads, self.key_dim)
                .permute(0, 2, 1, 3)
            )
...
class Levit(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = hybrid_backbone or stem_b16(
            in_chans, embed_dim[0], activation=act_layer
        )
        ...
        # Classifier head
        self.head = (
            NormLinear(embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()
        )
    ...
    def reset_classifier(self, num_classes, global_pool=None, distillation=None):
        ...
        if global_pool is not None:
            self.global_pool = global_pool
        self.head = (
            NormLinear(self.embed_dim[-1], num_classes)
            if num_classes > 0
            else nn.Identity()
        )
...
class LevitDistilled(Levit):
    def __init__(self, *args, **kwargs):
        ...
        self.head_dist = (
            NormLinear(self.num_features, self.num_classes)
    ...
    def reset_classifier(self, num_classes, global_pool=None, distillation=None):
        ...
        if global_pool is not None:
            self.global_pool = global_pool
        self.head = (
            NormLinear(self.num_features, num_classes)
            if num_classes > 0
            else nn.Identity()
        )
        self.head_dist = (
