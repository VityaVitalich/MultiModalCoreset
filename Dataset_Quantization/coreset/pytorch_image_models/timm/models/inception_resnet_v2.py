...
class InceptionResnetV2(nn.Module):
    def __init__(
        ...
        self.feature_info += [
            dict(num_chs=self.num_features, reduction=32, module="conv2d_7b")
        ]

        self.global_pool, self.classif = create_classifier(
    ...
    def group_matcher(self, coarse=False):
        module_map = {
            ...
            for i, (k, _) in enumerate(
                flatten_modules(self.named_children(), prefix=())
            )
        }
...
def inception_resnet_v2(pretrained=False, **kwargs):
    ...
    """
    return _create_inception_resnet_v2(
        "inception_resnet_v2", pretrained=pretrained, **kwargs
    )
...
def ens_adv_inception_resnet_v2(pretrained=False, **kwargs):
    ...
    """
    return _create_inception_resnet_v2(
        "ens_adv_inception_resnet_v2", pretrained=pretrained, **kwargs
    )
