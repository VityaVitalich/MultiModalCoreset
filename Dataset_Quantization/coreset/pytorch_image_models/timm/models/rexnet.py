...
class LinearBottleneck(nn.Module):
    def __init__(
        ...
        self.conv_dw = ConvNormAct(
            dw_chs, dw_chs, 3, stride=stride, groups=dw_chs, apply_act=False
        )
        if se_ratio > 0:
            self.se = SEWithNorm(
                dw_chs, rd_channels=make_divisible(int(dw_chs * se_ratio), ch_div)
            )
        else:
            self.se = None
    ...
    def forward(self, x):
        ...
        if self.use_shortcut:
            if self.drop_path is not None:
                x = self.drop_path(x)
            x = torch.cat(
                [x[:, 0 : self.in_channels] + shortcut, x[:, self.in_channels :]], dim=1
            )
        return x
...
def _build_blocks(
    ...
    for block_idx, (chs, exp_ratio, stride, se_ratio) in enumerate(block_cfg):
        if stride > 1:
            ...
            curr_stride *= stride
        block_dpr = (
            drop_path_rate * block_idx / (num_blocks - 1)
    ...
    feature_info += [
        dict(
            ...
            module=f"features.{len(features) - 1}",
        )
    ]
...
class ReXNetV1(nn.Module):
    def __init__(
        ...
        block_cfg = _block_cfg(
            width_mult, depth_mult, initial_chs, final_chs, se_ratio, ch_div
        )
        ...
        self.head = ClassifierHead(
            self.num_features, num_classes, global_pool, drop_rate
        )
...
def _create_rexnet(variant, pretrained, **kwargs):
    ...
    return build_model_with_cfg(
        ReXNetV1, variant, pretrained, feature_cfg=feature_cfg, **kwargs
    )
