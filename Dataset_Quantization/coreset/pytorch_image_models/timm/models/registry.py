...
def register_model(fn):
    ...
    has_valid_pretrained = (
        False  # check if model has a pretrained url to allow filtering on this
    )
...
def list_models(
    filter="", module="", pretrained=False, exclude_filters="", name_matches_cfg=False
):
...
def has_pretrained_cfg_key(model_name, cfg_key):
    ...
    if (
        model_name in _model_pretrained_cfgs
        and cfg_key in _model_pretrained_cfgs[model_name]
    ):
        return True
    return False
...
def is_pretrained_cfg_key(model_name, cfg_key):
    ...
    if model_name in _model_pretrained_cfgs and _model_pretrained_cfgs[model_name].get(
        cfg_key, False
    ):
        return True
    return False
