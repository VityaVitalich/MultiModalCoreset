...
class CspStagesCfg:
    depth: Tuple[int, ...] = (
        ...
        5,
        2,
    )  # block depth (number of block repeats in stages)
    out_chs: Tuple[int, ...] = (
        128,
        256,
        512,
        1024,
    )  # number of output channels for blocks in stage
    ...
    bottle_ratio: Union[
        float, Tuple[float, ...]
    ] = 1.0  # bottleneck-ratio of blocks in stage
    ...
    block_type: Union[
        str, Tuple[str]
    ] = "bottle"  # blocks type for stages ('bottle', 'dark')
...
def _cs3darknet_cfg(
    ...
    return CspModelCfg(
        ...
        stages=CspStagesCfg(
            out_chs=tuple(
                [make_divisible(c * width_multiplier) for c in (128, 256, 512, 1024)]
            ),
...
model_cfgs = dict(
    ...
    cs3darknet_focus_s=_cs3darknet_cfg(
        width_multiplier=0.5, depth_multiplier=0.5, focus=True
    ),
    cs3darknet_focus_m=_cs3darknet_cfg(
        width_multiplier=0.75, depth_multiplier=0.67, focus=True
    ),
    cs3darknet_focus_l=_cs3darknet_cfg(focus=True),
    cs3darknet_focus_x=_cs3darknet_cfg(
        width_multiplier=1.25, depth_multiplier=1.33, focus=True
    ),
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn2 = (
            create_attn(attn_layer, channels=mid_chs) if not attn_last else None
        )
        self.conv3 = ConvNormAct(
            mid_chs, out_chs, kernel_size=1, apply_act=False, **ckwargs
        )
...
class CrossStage(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        down_chs = (
            out_chs if down_growth else in_chs
        ...
        if stride != 1 or first_dilation != dilation:
            if avg_down:
                self.conv_down = nn.Sequential(
                    nn.AvgPool2d(2)
                    if stride == 2
                    else nn.Identity(),  # FIXME dilation handling
                    ConvNormActAa(
                        in_chs,
                        out_chs,
                        kernel_size=1,
        ...
        self.conv_exp = ConvNormAct(
            prev_chs, exp_chs, kernel_size=1, apply_act=not cross_linear, **conv_kwargs
        )
        ...
        # transition convs
        self.conv_transition_b = ConvNormAct(
            prev_chs, exp_chs // 2, kernel_size=1, **conv_kwargs
        )
        self.conv_transition = ConvNormAct(
...
class CrossStage3(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        prev_chs = (
            exp_chs // 2
        )  # expanded output is split in 2 for blocks and cross stage
...
def create_csp_stem(
    ...
    for i, chs in enumerate(out_chs):
        ...
        conv_stride = (
            2
            if (i == 0 and stride > 1) or (i == last_idx and stride > 2 and not pool)
            else 1
        )
        ...
        prev_feat = dict(
            num_chs=prev_chs,
            reduction=stem_stride,
            module=".".join(["stem", conv_name]),
        )
    if pool:
        assert stride > 2
...
def create_csp_stages(
    ...
    cfg_dict["block_dpr"] = (
        ...
        if not drop_path_rate
            ...
            x.tolist()
            for x in torch.linspace(0, drop_path_rate, sum(cfg.stages.depth)).split(
                cfg.stages.depth
            )
        ]
    )
    stage_args = [
        dict(zip(cfg_dict.keys(), values)) for values in zip(*cfg_dict.values())
    ]
    block_kwargs = dict(
        act_layer=cfg.act_layer, norm_layer=cfg.norm_layer, aa_layer=cfg.aa_layer
    )
...
class CspNet(nn.Module):
    ...
    def __init__(
        ...
        # Construct the stem
        self.stem, stem_feat_info = create_csp_stem(
            in_chans, **asdict(cfg.stem), **layer_args
        )
