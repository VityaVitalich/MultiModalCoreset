...
def _create_vision_transformer_hybrid(variant, backbone, pretrained=False, **kwargs):
    ...
    kwargs.setdefault(
        "patch_size", 1
    )  # default patch size for hybrid models if not set
    return _create_vision_transformer(
        variant, pretrained=pretrained, embed_layer=embed_layer, **kwargs
    )
...
def _resnetv2(layers=(3, 4, 9), **kwargs):
    ...
    conv_layer = (
        partial(StdConv2dSame, eps=1e-8)
        if padding_same
        else partial(StdConv2d, eps=1e-8)
    )
    if len(layers):
        backbone = ResNetV2(
            layers=layers,
