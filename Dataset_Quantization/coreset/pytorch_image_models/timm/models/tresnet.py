...
default_cfgs = {
    ...
    "tresnet_m_miil_in21k": _cfg(
        url="https://miil-public-eu.oss-eu-central-1.aliyuncs.com/model-zoo/ImageNet_21K_P/models/timm/tresnet_m_miil_in21k.pth",
        num_classes=11221,
    ),
    "tresnet_l": _cfg(
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, use_se=True, aa_layer=None
    ):
        ...
        self.se = (
            SEModule(planes * self.expansion, rd_channels=rd_chs) if use_se else None
        )
...
class TResNet(nn.Module):
    def __init__(
        ...
        layer1 = self._make_layer(
            BasicBlock, self.planes, layers[0], stride=1, use_se=True, aa_layer=aa_layer
        )  # 56x56
        ...
        self.feature_info = [
            ...
            dict(
                num_chs=self.planes * 4 * Bottleneck.expansion,
                reduction=16,
                module="body.layer3",
            ),
            dict(
                num_chs=self.planes * 8 * Bottleneck.expansion,
                reduction=32,
                module="body.layer4",
            ),
        ]
        ...
        for m in self.modules():
            if isinstance(m, BasicBlock):
                m.conv2[1].weight = nn.Parameter(
                    torch.zeros_like(m.conv2[1].weight)
                )  # BN to zero
            if isinstance(m, Bottleneck):
                m.conv3[1].weight = nn.Parameter(
                    torch.zeros_like(m.conv3[1].weight)
                )  # BN to zero
    ...
    def group_matcher(self, coarse=False):
        matcher = dict(
            stem=r"^body\.conv1",
            blocks=r"^body\.layer(\d+)" if coarse else r"^body\.layer(\d+)\.(\d+)",
        )
        return matcher
...
def tresnet_m_miil_in21k(pretrained=False, **kwargs):
    model_kwargs = dict(layers=[3, 4, 11, 3], **kwargs)
    return _create_tresnet(
        "tresnet_m_miil_in21k", pretrained=pretrained, **model_kwargs
    )
