...
def adjust_widths_groups_comp(widths, bottle_ratios, groups):
    ...
    bottleneck_widths = [
        quantize_float(w_bot, g) for w_bot, g in zip(bottleneck_widths, groups)
    ]
...
def downsample_conv(
    in_chs, out_chs, kernel_size=1, stride=1, dilation=1, norm_layer=None, preact=False
):
...
def downsample_avg(
    ...
    if preact:
        ...
        conv = ConvNormAct(
            in_chs, out_chs, 1, stride=1, norm_layer=norm_layer, apply_act=False
        )
...
def create_shortcut(
    ...
    if in_chs != out_chs or stride != 1 or dilation[0] != dilation[1]:
        dargs = dict(
            stride=stride, dilation=dilation[0], norm_layer=norm_layer, preact=preact
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if se_ratio:
            ...
            self.se = SEModule(
                bottleneck_chs, rd_channels=se_channels, act_layer=act_layer
            )
        else:
            self.se = nn.Identity()
        self.conv3 = ConvNormAct(
            bottleneck_chs, out_chs, kernel_size=1, apply_act=False, **cargs
        )
        self.act3 = nn.Identity() if linear_out else act_layer()
        self.downsample = create_shortcut(
            downsample, in_chs, out_chs, 1, stride, dilation, norm_layer=norm_layer
        )
        self.drop_path = (
...
class PreBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.downsample = create_shortcut(
            downsample, in_chs, out_chs, 1, stride, dilation, preact=True
        )
        self.drop_path = (
            DropPath(drop_path_rate) if drop_path_rate > 0 else nn.Identity()
        )
...
class RegNet(nn.Module):
    ...
    def __init__(
        ...
        for i, stage_args in enumerate(per_stage_args):
            ...
            self.add_module(
                stage_name,
                RegStage(
                    in_chs=prev_width, block_fn=block_fn, **stage_args, **common_args
                ),
            )
            prev_width = stage_args["out_chs"]
            curr_stride *= stage_args["stride"]
            self.feature_info += [
                dict(num_chs=prev_width, reduction=curr_stride, module=stage_name)
            ]

        # Construct the head
        if cfg.num_features:
            self.final_conv = ConvNormAct(
                prev_width, cfg.num_features, kernel_size=1, **na_args
            )
            self.num_features = cfg.num_features
        else:
            final_act = cfg.linear_out or cfg.preact
            self.final_conv = (
                get_act_layer(cfg.act_layer)() if final_act else nn.Identity()
            )
            self.num_features = prev_width
        self.head = ClassifierHead(
            in_chs=self.num_features,
    ...
    def _get_stage_args(
        self, cfg: RegNetCfg, default_stride=2, output_stride=32, drop_path_rate=0.0
    ):
