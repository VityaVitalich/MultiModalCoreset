...
def _download_from_hf(model_id: str, filename: str):
    ...
    return hf_hub_download(
        hf_model_id, filename, revision=hf_revision, cache_dir=get_cache_dir("hf")
    )
...
def save_for_hf(model, save_directory, model_config=None):
    ...
    hf_config["labels"] = model_config.pop(
        "labels", [f"LABEL_{i}" for i in range(hf_config["num_classes"])]
    )
