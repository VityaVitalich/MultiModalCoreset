...
async def rate_okey(message: types.Message):
    ...
    overall_rating += 3
    await message.reply("Спасибо! Мы постараемся улучшить наш сервис!", reply_markup=types.ReplyKeyboardRemove())
...
async def rate_bad(message: types.Message):
    ...
    overall_rating += 2
    await message.reply("Спасибо! Нам жаль, что сервис справился плохо, в скором времени станет лучше", reply_markup=types.ReplyKeyboardRemove())
...
async def rate_worst(message: types.Message):
    ...
    overall_rating += 1
    await message.reply("Спасибо! Нам очень жаль, что сервис не смог справиться, с последующими обновлениями качество вырастет", reply_markup=types.ReplyKeyboardRemove())