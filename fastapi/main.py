...
from model_utils import init_rgb_model, init_semseg_model, init_rgb_semseg_model, inference, semseg_inference, multi_inference, prepare_image, target_transform, masked_berhu_loss
app = FastAPI()
...
async def create_upload_file(file: UploadFile = File(...)):
    ...
    # Your logic here, e.g., process the image, save it, etc.
    # For example, let's just return the size of the image
    return {"filename": file.filename, "format": pil_image.format, "size": pil_image.size}


@app.get("/upload_two_image/")
async def create_upload_file(rgb: UploadFile, semseg: UploadFile):
    # Convert to PIL Image
    contents_rgb = await rgb.read()
    contents_semseg = await semseg.read()
    pil_image_rgb = Image.open(BytesIO(contents_rgb)).convert('RGB')
    pil_image_semseg = Image.open(BytesIO(contents_semseg))
    
    # Convert to NumPy array
    np_image = np.array(pil_image_rgb)
    s_image = np.array(pil_image_semseg)
    max_value = (np.max(np_image))

    # Your logic here, e.g., process the image, save it, etc.
    # For example, let's just return the size of the image
    return {"filename": rgb.filename, "format": pil_image_rgb.format, "size": pil_image_rgb.size, 'rgb_max': max_value.item(),
    "sformat": pil_image_semseg.format, "ssize": pil_image_semseg.size}
...
async def fine_tune_rgb(rgb_ls: List[UploadFile], depth_ls: List[UploadFile], token: str = Form(...)):
    # Convert to PIL Image
    if not token in authorized_tokens:
...
async def fine_tune_rgb(rgb: UploadFile):
    contents_rgb = await rgb.read()
    pil_image_rgb = Image.open(BytesIO(contents_rgb)).convert('RGB')
    sample_dict = prepare_image(pil_image_rgb)
    ...
    return {"message": "RGB embeddings successfully extracted", "embeddings": encoder_tokens.tolist()}